...
def save_to_csv(data, filename= "PENDLE-USDC_Data.csv"):
    file_exists = False
    try:
...
def main():
    ...
    #add your subpgrah api key here
    ...
    start_timestamp = int(datetime(2024, 4, 8, 23, 56, 20).timestamp())
    ...
    while current_timestamp <= today_timestamp:
        ...
        if pool_data:
            ...
            save_to_csv(csv_row, "PENDLE-USDC_Data.csv")
            print(f"Row written: {csv_row}")
            
            #increment time weekly
